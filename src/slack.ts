...
import { blocks, modal, slackApi, validateSlackRequest } from './util/slack';
		...
		case '/foodfight':
			...
			const response = await slackApi('views.open', 
				modal({
					...
					blocks: [
						blocks.section({
							text: 'The discourse demands food drama! *Send in your spiciest food takes!*'
						}),
						blocks.input({
							id: 'opinion',
							label: 'Deposit your controversial food opinions here.',
							placeholder: 'Example: peanut butter and mayo sandwiches are delicious!',
							initial_value: payload.text ?? '',
							hint: 'What do you believe about food that people find appalling?'
						}),
						blocks.select({
							id: 'spice_level',
	...
	switch(callback_id){
		case 'foodfight-modal':
			...
			await slackApi('chat.postMessage', {
				channel: 'C05SL27HEG3',
				text: `Oh dang, y'all! :eyes: <@${payload.user.username} just started a food fight with a ${fields.spiceLevel} take:\n\n*${fields.opinion}*\n\n...discuss.`
			});
			break;

		case 'start_food_fight_nudge':
...
export const handler: Handler = async (event) => {
	...
	const valid = validateSlackRequest(event);
